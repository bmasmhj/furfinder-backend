-- Content Management Tables for FurFinder Website
-- This script adds tables for blogs, FAQs, features, pricing, how-it-works, who-its-for, and reunited stories
...
CREATE TABLE IF NOT EXISTS blogs (
  ...
  author_name VARCHAR(255) NOT NULL DEFAULT 'FurFinder Team',
...
-- Create who_its_for_segments table
CREATE TABLE IF NOT EXISTS who_its_for_segments (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  title VARCHAR(255) NOT NULL,
  description TEXT NOT NULL,
  icon_name VARCHAR(100),
...
CREATE INDEX IF NOT EXISTS idx_features_active ON features(is_active, display_order);
CREATE INDEX IF NOT EXISTS idx_pricing_active ON pricing_plans(is_active, display_order);
CREATE INDEX IF NOT EXISTS idx_how_it_works ON how_it_works_steps(is_active, step_number);
CREATE INDEX IF NOT EXISTS idx_who_its_for ON who_its_for_segments(is_active, display_order);
CREATE INDEX IF NOT EXISTS idx_reunited_slug ON reunited_stories(slug);
CREATE INDEX IF NOT EXISTS idx_reunited_published ON reunited_stories(is_published, featured_on_homepage);
