...
export const metadata: Metadata = {
  ...
  description: 'Practical advice, stories from the search, and product updates from Australia\'s AI-powered pet recovery platform.',
}
    ...
    <div className="min-h-screen bg-surface">
      <main>
        ...
        <section className="bg-surface px-6 py-20 border-b">
          <div className="max-w-7xl mx-auto">
             ...
             <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight text-[#1a1a2e] mt-4 mb-6">
               Stories, updates & <span className="text-[#ff6b4a]">pet advice.</span>
             </h1>
        ...
        <div className="max-w-7xl mx-auto px-6 py-16">
          {featuredPost && (
            <div className="mb-20">
              <Link href={`/blog/${featuredPost.slug}`} className="group relative block overflow-hidden rounded-3xl card-bg border shadow-sm hover:shadow-xl transition-all duration-300">
                <div className="grid md:grid-cols-2">
                  ...
                  <div className="p-8 md:p-12 flex flex-col justify-center">
                    <div className="flex items-center gap-4 text-sm text-[#9ca3af] mb-4">
                      <span className="font-semibold text-[#ff6b4a] uppercase tracking-wider">{featuredPost.category}</span>
                      <span>•</span>
                    ...
                    <div className="flex items-center gap-3">
                      {featuredPost.author_image && (
                         <img src={featuredPost.author_image} alt={featuredPost.author_name} className="w-10 h-10 rounded-full object-cover" />
                      )}
                      <div>
                        <span className="block text-sm font-bold text-[#1a1a2e]">{featuredPost.author_name || 'The Fur Finder Team'}</span>
                        <span className="block text-xs text-[#9ca3af]">{featuredPost.read_time || '5 min read'}</span>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          {remainingPosts.length > 0 ? (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {remainingPosts.map((post: any) => (
                <Link key={post.id} href={`/blog/${post.slug}`} className="group flex flex-col card-bg border rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300">
                  ...
                  <div className="p-8 flex flex-col flex-grow">
                    <div className="flex items-center gap-3 text-xs text-[#9ca3af] mb-4">
                      <span className="font-bold text-[#ff6b4a] uppercase tracking-wider">{post.category}</span>
                      <span>•</span>
                    ...
                    <div className="flex items-center justify-between pt-6 border-t border-[#f3f4f6]">
                       <span className="text-xs font-semibold text-[#1a1a2e]">{post.author_name || 'Admin'}</span>
                       <span className="text-xs text-[#9ca3af]">{post.read_time || '4 min read'}</span>
                    </div>
                  </div>
                </Link>
              ))}
            ...
            <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-[#d1d5db]">
              <p className="text-[#6b7280] text-lg">New stories are coming soon. Stay tuned!</p>
            </div>
          )}
